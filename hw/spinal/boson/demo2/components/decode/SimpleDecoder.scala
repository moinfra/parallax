package boson.demo2.components.decode
...
class SimpleDecoder extends Component {
  ...
  switch(fields.opcode) {
    is(InstructionOpcodes.LD) {
      ...
      io.microOp.memInfo.accessType := MemoryAccessType.LOAD_U
      io.microOp.memInfo.size := MemoryOpSize.WORD
    }
    is(InstructionOpcodes.ST) {
      ...
      io.microOp.memInfo.accessType := MemoryAccessType.STORE
      io.microOp.memInfo.size := MemoryOpSize.WORD
    }
    is(InstructionOpcodes.ADD) {
      ...
      io.microOp.aluInfo.op := AluOp.ADD
    }
    is(InstructionOpcodes.ADDI) {
      ...
      io.microOp.aluInfo.op := AluOp.ADD // ALU performs Rs + Imm
    }
    is(InstructionOpcodes.NEG) { // Rd = -Rs (0 - Rs)
      ...
      io.microOp.aluInfo.op := AluOp.SUB // ALU will compute 0 - archRegRs_value
    }
    is(InstructionOpcodes.AND) {
      ...
      io.microOp.aluInfo.op := AluOp.AND
    }
    is(InstructionOpcodes.OR) {
      ...
      io.microOp.aluInfo.op := AluOp.OR
    }
    is(InstructionOpcodes.XOR) {
      ...
      io.microOp.aluInfo.op := AluOp.XOR
    }
    is(InstructionOpcodes.NOT) { // Rd = ~Rs (Rs XOR -1)
      ...
      io.microOp.aluInfo.op := AluOp.XOR // ALU will compute archRegRs_value XOR S(-1)
    }
    is(InstructionOpcodes.MUL) {
      ...
      io.microOp.aluInfo.op := AluOp.MUL
    }
    is(InstructionOpcodes.DIV) {
      ...
      io.microOp.aluInfo.op := AluOp.DIV
    }
    is(InstructionOpcodes.LI) { // Rd = sext(Imm)
      ...
      io.microOp.aluInfo.op := AluOp.ADD // Assuming ALU does 0 + imm for LI
    }
    is(InstructionOpcodes.EQ) {
      ...
      io.microOp.aluInfo.op := AluOp.EQ
    }
    is(InstructionOpcodes.SGT) {
      ...
      io.microOp.aluInfo.op := AluOp.SGT
    }
