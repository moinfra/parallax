...
class DecoderComponent(pCfg: BosonConfig) extends Component {
  ...
  val imm_offs21 = signExtend(
    i_imm21_raw(4 downto 0) ## i_imm21_raw(20 downto 5) ## B"00",
    23
  ) // {offs[20:16], offs[15:0], 2'b0} signed
  val imm_offs26 = signExtend(
    i_imm26_raw(9 downto 0) ## i_imm26_raw(25 downto 10) ## B"00",
    28
  ...
  report(L"Start decoding instruction: ${io.instruction}")
  // --- Helper Functions for Populating DecodedInst ---
  def setDestGpr(idx: UInt): Unit = {
    d.archDest.idx := idx.resized
  ...
  def setDestCfr(idx: UInt): Unit = {
    d.archDest.idx := idx(2 downto 0).resized // CFR index is smaller
    d.archDest.isFpr := False
    d.archDest.isCfr := True
    d.writeDestEn := True // Assuming no zero CFR
  }
  // Source setters...
  def setSrc1Gpr(idx: UInt): Unit = {
    d.archSrc1.idx := idx.resized; d.archSrc1.isFpr := False; d.archSrc1.isCfr := False; d.useSrc1 := True
  }
  def setSrc1Fpr(idx: UInt): Unit = {
    d.archSrc1.idx := idx.resized; d.archSrc1.isFpr := True; d.archSrc1.isCfr := False; d.useSrc1 := True
  }
  def setSrc1Cfr(idx: UInt): Unit = {
    d.archSrc1.idx := idx(2 downto 0).resized; d.archSrc1.isFpr := False; d.archSrc1.isCfr := True; d.useSrc1 := True
  }
  def setSrc2Gpr(idx: UInt): Unit = {
    d.archSrc2.idx := idx.resized; d.archSrc2.isFpr := False; d.archSrc2.isCfr := False; d.useSrc2 := True
  }
  def setSrc2Fpr(idx: UInt): Unit = {
    d.archSrc2.idx := idx.resized; d.archSrc2.isFpr := True; d.archSrc2.isCfr := False; d.useSrc2 := True
  }
  def setSrc2Cfr(idx: UInt): Unit = {
    d.archSrc2.idx := idx(2 downto 0).resized; d.archSrc2.isFpr := False; d.archSrc2.isCfr := True; d.useSrc2 := True
  }
  def setSrc3Gpr(idx: UInt): Unit = {
    d.archSrc3.idx := idx.resized; d.archSrc3.isFpr := False; d.archSrc3.isCfr := False; d.useSrc3 := True
  }
  def setSrc3Fpr(idx: UInt): Unit = {
    d.archSrc3.idx := idx.resized; d.archSrc3.isFpr := True; d.archSrc3.isCfr := False; d.useSrc3 := True
  }
  def setSrc3Cfr(idx: UInt): Unit = {
    d.archSrc3.idx := idx(2 downto 0).resized; d.archSrc3.isFpr := False; d.archSrc3.isCfr := True; d.useSrc3 := True
  }
  def setUopCode(uop: SpinalEnumElement[UopCode.type]): Unit = {
    d.uopCode := uop
    report(L"Set uopCode to ${uop.name}")
  }
  ...
  switch(io.instruction) {
    ...
    is(M"0000000_0000_100000-----_-----_-----") { // ADD.W
      setUopCode(UopCode.ALU_ADD); d.executeUnit := ExeUnit.ALU
    ...
    is(M"0000000_0000_100010-----_-----_-----") { // SUB.W
      setUopCode(UopCode.ALU_ADD); d.executeUnit := ExeUnit.ALU // Use ALU_ADD with isSub flag
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.aluFlags.isSub := True; d.aluFlags.isUns := False
    }
    is(M"0000000_0000_100100-----_-----_-----") { // SLT
      setUopCode(UopCode.ALU_SLT); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.aluFlags.isSub := False; d.aluFlags.isUns := False
    }
    is(M"0000000_0000_100101-----_-----_-----") { // SLTU
      setUopCode(UopCode.ALU_SLT); d.executeUnit := ExeUnit.ALU // Use ALU_SLT with isUns flag
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.aluFlags.isSub := False; d.aluFlags.isUns := True
    }
    // Integer Logic (Reg-Reg)
    is(M"0000000_0000_101001-----_-----_-----") { // AND
      setUopCode(UopCode.ALU_AND); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    }
    is(M"0000000_0000_101010-----_-----_-----") { // OR
      setUopCode(UopCode.ALU_OR); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    }
    is(M"0000000_0000_101000-----_-----_-----") { // NOR
      setUopCode(UopCode.ALU_NOR); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    }
    is(M"0000000_0000_101011-----_-----_-----") { // XOR
      setUopCode(UopCode.ALU_XOR); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    }

    // Integer Shifts (Reg-Reg) - Use only lower 5 bits of rk
    is(M"0000000_0000_101110-----_-----_-----") { // SLL.W
      setUopCode(UopCode.SHIFT_L); d.executeUnit := ExeUnit.ALU // Or dedicated SHIFT unit
    ...
    is(M"0000000_0000_101111-----_-----_-----") { // SRL.W
      setUopCode(UopCode.SHIFT_R); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.shiftFlags.isRight := True; d.shiftFlags.isArith := False
    }
    is(M"0000000_0000_110000-----_-----_-----") { // SRA.W
      setUopCode(UopCode.SHIFT_RA); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    ...
    is(M"0000000_0000_111000-----_-----_-----") { // MUL.W
      setUopCode(UopCode.MUL_W); d.executeUnit := ExeUnit.MUL
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    }
    is(M"0000000_0000_111001-----_-----_-----") { // MULH.W
      setUopCode(UopCode.MULH_W); d.executeUnit := ExeUnit.MUL
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    }
    is(M"0000000_0000_111010-----_-----_-----") { // MULH.WU
      setUopCode(UopCode.MULH_WU); d.executeUnit := ExeUnit.MUL
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
    }
    is(M"0000000_0001_000000-----_-----_-----") { // DIV.W
      setUopCode(UopCode.DIV_W); d.executeUnit := ExeUnit.DIV
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.mayCauseExc := True // Division by zero
    }
    is(M"0000000_0001_000001-----_-----_-----") { // MOD.W
      setUopCode(UopCode.MOD_W); d.executeUnit := ExeUnit.DIV
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.mayCauseExc := True // Division by zero
    }
    is(M"0000000_0001_000010-----_-----_-----") { // DIV.WU
      setUopCode(UopCode.DIV_WU); d.executeUnit := ExeUnit.DIV
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.mayCauseExc := True // Division by zero
    }
    is(M"0000000_0001_000011-----_-----_-----") { // MOD.WU
      setUopCode(UopCode.MOD_WU); d.executeUnit := ExeUnit.DIV
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw); setSrc2Gpr(i_rk_raw)
      d.mayCauseExc := True // Division by zero
    }
    ...
    is(M"0000000_0001_010100-----_-----_-----") { // BREAK
      setUopCode(UopCode.BREAK); d.executeUnit := ExeUnit.CSR // Or specific System unit
      d.sysCode := i_imm15_raw.asBits
      d.generatesExc := True
      d.excCode := ExceptionCode.BRK
    }
    is(M"0000000_0001_010110-----_-----_-----") { // SYSCALL
      setUopCode(UopCode.SYSCALL); d.executeUnit := ExeUnit.CSR
    ...
    is(M"0000001_001------------_-----_-----") { // SLTUI
      ...
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw);
      d.imm := si12;
      d.useImmAsSrc2 := True // ISA says si12, but comparison is unsigned- Check ISA doc. Assuming unsigned compare against sign-extended imm.
      d.aluFlags.isSub := False; d.aluFlags.isUns := True
    }
    ...
    is(M"0000000_0010_000001-----_-----_-----") { // SLLI.W
      setUopCode(UopCode.SHIFT_L); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw)
    ...
    is(M"0001010_--------------------_-----") { // LU12I.W
      setUopCode(UopCode.ALU_LU12I); d.executeUnit := ExeUnit.ALU
      setDestGpr(i_rd_raw)
      d.imm := imm_lu12i // Special format {si20, 12'b0}
      // Operation is just GR[rd] = imm
    }
    is(M"0001110_--------------------_-----") { // PCADDU12I
      setUopCode(UopCode.ALU_PCADDU12I); d.executeUnit := ExeUnit.ALU // Needs PC as input
      setDestGpr(i_rd_raw)
    ...
    is(M"010110----------------_-----_-----") { // BEQ
      setUopCode(UopCode.BRANCH); d.executeUnit := ExeUnit.BRU
    ...
    is(M"010011----------------_-----_-----") { // JIRL rd, rj, offs16
      setUopCode(UopCode.JUMP_REG); d.executeUnit := ExeUnit.BRU
    ...
    is(M"0010100_000------------_-----_-----") { // LD.B
      setUopCode(UopCode.LOAD); d.executeUnit := ExeUnit.LSU
    ...
    is(M"0010100_010------------_-----_-----") { // LD.W
      ...
      setDestGpr(i_rd_raw); setSrc1Gpr(i_rj_raw)
      d.imm := si12; d.useImmAsSrc2 := True
      d.memSize := MemSize.W; d.memFlags.isSigned := False;
      d.memFlags.isUns := False // Word load is not signed/unsigned
      d.mayCauseExc := True // Alignment, Page Fault
    }
    ...
    is(M"0010100_100------------_-----_-----") { // ST.B
      setUopCode(UopCode.STORE); d.executeUnit := ExeUnit.LSU
    ...
    is(M"0010000_0--------------_-----_-----") { // LL.W (si14 offset needs checking)
      setUopCode(UopCode.ATOMIC_LL); d.executeUnit := ExeUnit.LSU
    ...
    is(M"0010000_1--------------_-----_-----") { // SC.W
      setUopCode(UopCode.ATOMIC_SC); d.executeUnit := ExeUnit.LSU
      setDestGpr(i_rd_raw) // Destination rd receives success/fail (1/0)
      setSrc1Gpr(i_rj_raw) // Addr base rj
      setSrc2Gpr(
        i_rd_raw
      ) // Data to store comes from rd (overwritten on success/fail) -> needs careful renaming/handling
    ...
    is(M"0011100_001_1100100_---------------") { // DBAR
      setUopCode(UopCode.MEM_BARRIER); d.executeUnit := ExeUnit.LSU
      d.hintOrCacheOp := io.instruction(14 downto 0).asBits // hint
    }
    is(M"0011100_001_1100101_---------------") { // IBAR
      setUopCode(UopCode.MEM_BARRIER);
      d.executeUnit := ExeUnit.LSU // Instruction barrier needs special handling in fetch/frontend
      d.hintOrCacheOp := io.instruction(14 downto 0).asBits // hint
    }
    is(M"0010101_011------------_-----_-----") { // PRELD
      setUopCode(UopCode.PREFETCH); d.executeUnit := ExeUnit.LSU
      setSrc1Gpr(i_rj_raw) // Base address
      d.imm := si12; d.useImmAsSrc2 := True // Offset
      d.hintOrCacheOp := io.instruction(4 downto 0).asBits.resized // TODO: hint 这里再仔细看看，因为本来应该 15 bits
    }

    // CSR Instructions - Opcode 0000010
    is(M"0000010_0_--------------_00000_-----") { // CSRRD rd, csr
      setUopCode(UopCode.CSR_READ); d.executeUnit := ExeUnit.CSR
      setDestGpr(i_rd_raw)
      d.csrAddr := i_csr_raw.asBits
      d.csrFlags.isWrite := False; d.csrFlags.isXchg := False
      d.mayCauseExc := True // Privilege
    }
    is(M"0000010_0_--------------_00001_-----") { // CSRWR rd, csr (rd seems unused based on pseudocode- Verify)
      setUopCode(UopCode.CSR_WRITE); d.executeUnit := ExeUnit.CSR
    ...
    is(M"0000010_0_--------------__-----_-----") { // CSRXCHG rd, rj, csr (matches if rj != 00000 or 00001)
      // Need to exclude CSRRD/CSRWR patterns if they overlap
      when(i_rj_raw =/= 0 && i_rj_raw =/= 1) {
        setUopCode(UopCode.CSR_XCHG); d.executeUnit := ExeUnit.CSR
    ...
    is(M"0000000_1000_000001-----_-----_-----") { // FADD.S
      setUopCode(UopCode.FPU_ADD); d.executeUnit := ExeUnit.FPU_ADD
    ...
    is(M"0000000_1000_001001-----_-----_-----") { // FMUL.S
      setUopCode(UopCode.FPU_MUL); d.executeUnit := ExeUnit.FPU_MUL
      setDestFpr(i_fd_raw); setSrc1Fpr(i_fj_raw); setSrc2Fpr(i_fk_raw)
    ...
    is(M"0000000_1000_001101-----_-----_-----") { // FDIV.S
      setUopCode(UopCode.FPU_DIV); d.executeUnit := ExeUnit.FPU_DIV
      setDestFpr(i_fd_raw); setSrc1Fpr(i_fj_raw); setSrc2Fpr(i_fk_raw)
      d.fpSize := FpSize.S; d.mayCauseExc := True
    }
    ...
    is(M"0000100_00001-----_-----_-----_-----") { // FMADD.S
      setUopCode(UopCode.FPU_FMA); d.executeUnit := ExeUnit.FPU_MUL
    ...
    is(M"0000110_00001-----_-----_-----_-----") { // FCMP.cond.S
      setUopCode(UopCode.FPU_CMP); d.executeUnit := ExeUnit.FPU_ADD // Often shares adder path
    ...
    is(M"0000110_00010-----_-----_-----_-----") { // FCMP.cond.D
      setUopCode(UopCode.FPU_CMP); d.executeUnit := ExeUnit.FPU_ADD
      setDestCfr(i_cd_raw); setSrc1Fpr(i_fj_raw); setSrc2Fpr(i_fk_raw)
      d.fpSize := FpSize.D
    ...
    is(M"0000110_1000000---_-----_-----_-----") { // FSEL fd, fj, fk, ca (ca is in rd field bits 2:0)
      setUopCode(UopCode.FPU_SEL); d.executeUnit := ExeUnit.FPU_ADD // Simple select often on adder path
      setDestFpr(i_fd_raw); setSrc1Fpr(i_fj_raw); setSrc2Fpr(i_fk_raw);
      setSrc3Cfr(i_rd_raw) // Src3 is condition flag 'ca'
    ...
    is(M"0000000_1000_10100101001_-----_-----") { // MOVGR2FR.W fd, rj
      setUopCode(UopCode.MOV_GPR2FPR); d.executeUnit := ExeUnit.ALU // Or FPU_MISC
      setDestFpr(i_fd_raw); setSrc1Gpr(i_rj_raw)
      d.fpSize := FpSize.S // Affects lower 32 bits of FPR
    }
    is(M"0000000_1000_10100101101_-----_-----") { // MOVFR2GR.S rd, fj
      setUopCode(UopCode.MOV_FPR2GPR); d.executeUnit := ExeUnit.ALU
    ...
    is(M"0000000_1000_11001000110_-----_-----") { // FCVT.S.D fd, fj
      setUopCode(UopCode.FPU_CVT_F2F); d.executeUnit := ExeUnit.FPU_ADD
    ...
    is(M"0000000_1000_11101000100_-----_-----") { // FFINT.S.W fd, fj (fj interpreted as GPR content via FPR)
      setUopCode(UopCode.FPU_CVT_I2F); d.executeUnit := ExeUnit.FPU_ADD
    ...
    is(M"0000000_1000_11011000001_-----_-----") { // FTINT.W.S fd, fj (Result is Int bits in FPR)
      setUopCode(UopCode.FPU_CVT_F2I); d.executeUnit := ExeUnit.FPU_ADD
      setDestFpr(i_fd_raw); setSrc1Fpr(i_fj_raw)
      d.fpSize := FpSize.S
    ...
    is(M"0000000_0000_00000011000_00000_-----") { // RDCNTVL.W rd
      setUopCode(UopCode.RD_CNT_VL); d.executeUnit := ExeUnit.CSR // Or dedicated counter logic
      setDestGpr(i_rd_raw)
    }
    is(M"0000000_0000_00000011001_00000_-----") { // RDCNTVH.W rd
      setUopCode(UopCode.RD_CNT_VH); d.executeUnit := ExeUnit.CSR
      setDestGpr(i_rd_raw)
    }
    is(M"0000000_0000_00000011000_-----_00000") { // RDCNTID.W rj
      setUopCode(UopCode.RD_CNT_ID); d.executeUnit := ExeUnit.CSR
    ...
    default {
      report(L"Illegal Instruction: ${io.instruction}")
      d.isValid := False // Mark as invalid instruction
      setUopCode(UopCode.NOP)
      d.generatesExc := True
