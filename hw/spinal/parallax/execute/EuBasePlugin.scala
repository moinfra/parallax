...
// --- EU 寄存器文件类型枚举 ---
object EuRegType extends SpinalEnum {
  val GPR_ONLY, FPR_ONLY, MIXED_GPR_FPR, NONE = newElement()
}
...
abstract class EuBasePlugin(
  ...
  protected def euSpecificContextType: HardType[T_EuSpecificContext] // 返回 EU 特定上下文的 HardType
  protected def euRegType: EuRegType.C // 指定此 EU 与哪些物理寄存器文件交互

  // 派生 EU 必须实现这些方法来定义其微流水线
  protected def buildMicroPipeline(pipeline: Pipeline): Unit
  protected def connectPipelineLogic(pipeline: Pipeline): Unit
  // 派生 EU 必须返回其结果准备就绪的最终阶段
  protected def getWritebackStage(pipeline: Pipeline): Stage


  // --- 通用基础设施 ---
  lazy val commonSignals = ParallaxEuCommonSignals(pipelineConfig)
  // EU 流水线输入 Payload 的 Stageable
  // 使用 lazy val 确保 euSpecificContextType 在子类构造函数中首先初始化
  lazy val EU_INPUT_PAYLOAD = Stageable(commonSignals.EuPushPortPayload(euSpecificContextType))
  ...
  // --- 服务依赖 (使用 lazy 实现延迟获取) ---
  protected lazy val gprFileService: PhysicalRegFileService = {
    if (euRegType == EuRegType.GPR_ONLY || euRegType == EuRegType.MIXED_GPR_FPR) {
      findService[PhysicalRegFileService](_.isGprService()) // findService 需要一个过滤器
    } else null // 如果 EU 类型不涉及 GPR，则为 null
  }

  protected lazy val fprFileService: PhysicalRegFileService = {
  ...
  protected lazy val robService: ROBService[RU_TYPE_FOR_ROB] = getService[ROBService[RU_TYPE_FOR_ROB]]
  // BypassService 现在是泛型的，我们需要指定 Payload 类型 (BypassMessage)
  protected lazy val bypassService: BypassService[BypassMessage] = getService[BypassService[BypassMessage]]


  // --- PRF 读/写端口 ---
  protected val numGprReadPortsPerEu = 2 // 每个 EU 实例的 GPR 读端口数量 (例如，用于处理两个源操作数)
  protected val numFprReadPortsPerEu = 2 // 每个 EU 实例的 FPR 读端口数量

  protected lazy val gprReadPorts: Seq[PrfReadPort] =
    if (euRegType == EuRegType.GPR_ONLY || euRegType == EuRegType.MIXED_GPR_FPR) {
  ...
  protected lazy val fprReadPorts: Seq[PrfReadPort] =
    if (pipelineConfig.hasFpu && (euRegType == EuRegType.FPR_ONLY || euRegType == EuRegType.MIXED_GPR_FPR)) {
  ...
  // EU 将驱动此 Bundle 的字段。
  protected lazy val robWritebackPortBundle: ROBWritebackPort[RU_TYPE_FOR_ROB] = robService.newWritebackPort(euName)
  
  // --- 旁路输出端口 (基于 Flow) ---
  protected lazy val bypassOutputPort: Flow[BypassMessage] = bypassService.newBypassSource(euName)
  ...
  // --- 流水线构建 ---
  // EU 的内部微流水线
  val euPipeline = create early new Pipeline { setName(s"${euName}_internal_pipeline") }
  ...
  val setupPhase = create early new Area {
    ...
    if (pipelineConfig.hasFpu && (euRegType == EuRegType.FPR_ONLY || euRegType == EuRegType.MIXED_GPR_FPR) && fprFileService == null) {
  ...
  val logicPhase = create late new Area {
    ...
    val uopFullPayloadAtWb = wbStage(EU_INPUT_PAYLOAD) // 获取该阶段的完整 Uop 输入
  ...
  protected def connectGprRead(
      ...
      prfDataTarget: Stageable[Bits] // `stage` 中的 Stageable，用于存储读取的数据
  ): Unit = {
    if (euRegType == EuRegType.FPR_ONLY || euRegType == EuRegType.NONE || gprFileService == null) {
      ...
      ParallaxLogger.error(s"$euName (类型: $reason) 尝试 GPR 读取。设计错误。")
      stage(prfDataTarget).assignDontCare()
      when(stage.isFiring && useThisSrcSignal) { // isFiring 表示阶段正在有效处理
          report(L"SIM_ERROR: $euName: 非 GPR EU 或 GPR 服务缺失时尝试 GPR 读取。物理寄存器: ${physRegIdx}")
          stage.haltIt() // 错误时暂停阶段
      }
      return
    }
    if (prfReadPortIdx < gprReadPorts.length) { // 检查端口索引是否在范围内
        ...
        stage(prfDataTarget) := gprPort.rsp // 假设 PRF 读取延迟由流水线结构或暂停处理
                                            // 对于组合读或0周期延迟，数据可直接用于当前阶段逻辑
    } else {
        ParallaxLogger.error(s"$euName: 请求的 GPR 读端口索引 $prfReadPortIdx 超出范围 (${gprReadPorts.length} 可用)。")
        stage(prfDataTarget).assignDontCare()
        when(stage.isFiring && useThisSrcSignal) {
          report(L"SIM_ERROR: $euName: GPR 读端口索引 ${prfReadPortIdx.toString} 超出范围。物理寄存器: ${physRegIdx}")
          stage.haltIt()
        }
    }
  }

  protected def connectFprRead(
      stage: Stage,
      prfReadPortIdx: Int,
      physRegIdx: UInt,
