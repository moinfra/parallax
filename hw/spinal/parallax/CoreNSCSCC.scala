...
/** A service to provide final, calculated signals for simulation-only debugging.
  */
trait SimDebugService extends Service {

  /** Returns a Vec representing the current state of the architectural GPRs.
    * This signal is calculated within the SimDebugPlugin.
    */
  def getArchGprs(): Vec[Bits]
}

/** A plugin that performs all logic required to export the architectural
  * state of the CPU for simulation purposes.
  *
  * It acts as a "man-in-the-middle" for simulation, observing internal
  * services (RAT, PRF) and computing a final, easily-accessible signal.
  *
  * @param pCfg The pipeline configuration.
  */
class SimDebugPlugin(pCfg: PipelineConfig) extends Plugin with SimDebugService {

  // A place to hold the final calculated signal.
  private var archGprs: Vec[Bits] = null

  val setup = create early new Area {
    val prfService = framework.getService[PhysicalRegFileService]
    val prfReadPorts = Vec(
      (0 until pCfg.archGprCount).map(_ => prfService.newDebuggerReadPort())
    )

    // Now, let's connect the logic in the `late` area using these ports.
  }
  val logic = create late new Area {
    val prfReadPorts = setup.prfReadPorts

    val ratControlService = framework.getService[RatControlService]

    val currentRatState = ratControlService.getCurrentState()
    val ratMapping = currentRatState.mapping

    // Calculate the architectural GPRs
    val archGprs_internal = Vec(Bits(pCfg.dataWidth), pCfg.archGprCount)
    for (i <- 0 until pCfg.archGprCount) {
      val physRegAddr = ratMapping(i)
      val readPort = prfReadPorts(i)
      readPort.valid := True
      readPort.address := physRegAddr

      archGprs_internal(i) := readPort.rsp
    }

    // Store the final signal in our private variable.
    archGprs = archGprs_internal
  }

  // The implementation of the service method.
  override def getArchGprs(): Vec[Bits] = {
    framework.requireLate()
    assert(archGprs != null, "ArchGprs signal was not generated by SimDebugPlugin!")
    archGprs
  }
}
...
class CoreNSCSCC(simDebug: Boolean = false, injectAxi: Boolean = false) extends Component {
  ...
  def plugins() = {
    ...
    if (simDebug) {
      _plugins += new SimDebugPlugin(pCfg)
    } else if (simDebug && injectAxi) {
  ...
  simDebug generate {
    ...
    commitLog = commitService.commitSlotLogs
    commitLog.foreach(_.simPublic)
  }

  var simArchGprs: Option[Vec[Bits]] = None
  simDebug generate {
    val simDebugService = framework.getService[SimDebugService]
    val archGprs_internal = simDebugService.getArchGprs()

    archGprs_internal.simPublic()
    simArchGprs = Some(archGprs_internal)
  }
