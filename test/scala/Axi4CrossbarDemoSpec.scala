...
class Axi4SlaveFactorySpec extends SpinalSimFunSuite { // Or just add test to Axi4CrossbarDemoSpec
...
class Axi4SharedToFullConverterSpec extends SpinalSimFunSuite { // Or as part of Axi4CrossbarDemoSpec
  ...
  // --- Axi4SharedToFullConverter (using the new logic) ---
...
class SplitGmbToAxi4BridgeSpec extends SpinalSimFunSuite {
  ...
  test("SplitGmbToAxi4Bridge_UnitTest") {
    ...
    SimConfig.withWave.compile(new Component {
      ...
      println(s"SplitBridgeTest @${simTime()-10}: GMB Write CMD accepted by bridge (ID=$writeId).")
      ...
      println(s"SplitBridgeTest @${simTime()-10}: GMB Write Rsp received: Error=${gmbWriteRsp.payload.error.toBoolean}, ID=${if(GMB_SPLIT_CONFIG.useId && gmbWriteRsp.payload.id != null) gmbWriteRsp.payload.id.toBigInt else -1}")
      ...
      println(s"SplitBridgeTest @${simTime()-10}: GMB Read CMD accepted (ID=$readId).")
      ...
      println(s"SplitBridgeTest @${simTime()-10}: GMB Read Rsp received: Data=0x${rdata.toString(16)}, Error=${gmbReadRsp.payload.error.toBoolean}, ID=${rid}")
