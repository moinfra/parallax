...
// import boson.demo2.common.Config // Assuming Config.XLEN is defined (not used in these tests directly)
...
if(false) {
  test("SimulatedMemory should respond to a single word read request") {
    ...
    test("SimulatedMemory should respond to a single word write request") {
      ...
      val memSizeBytes = 1024
      val latencyCycles = 1
      val timeout = latencyCycles + 20 // Adjusted timeout
    ...
    test("SimulatedMemory should handle back-to-back write requests followed by reads") {
      ...
      simConfig
        ...
        .doSim { dut =>
          ...
          testData.foreach { case (_, expectedData) =>
            assert(receivedResponses.nonEmpty, "Not enough read responses")
            val (readData, error) = receivedResponses.dequeue()
            assert(!error, s"Error flag set for read of $expectedData")
            assert(readData == expectedData, s"Read back data mismatch. Expected $expectedData, got $readData")
          }
          dut.clockDomain.waitEdge(10)
        }
    }
    ...
    test("SimulatedMemory should handle mixed read and write requests") {
      ...
      val memSizeBytes = 1024
      val latencyCycles = 2
      val timeout = (latencyCycles + 15) * 5 // 5 operations
  ...
  if (false) {
    ...
    test("SimulatedMemory should signal error when internalDataWidth > busDataWidth (unsupported)") {
      // This scenario is typically an error or requires more complex bus (e.g. byte enables, size field)
      // for the bus to access slices of a wider internal memory word.
      // Assuming SimulatedMemory signals an error for this configuration mismatch.
      val internalMemWidthLocal = 32
      val busDataWidthLocal = 16
      val memSizeBytes = 256
      val latencyCycles = 1
      val timeout = latencyCycles + 20

      val memCfg = SimulatedMemoryConfig(
        internalDataWidth = internalMemWidthLocal,
        memSize = memSizeBytes,
        initialLatency = latencyCycles
      )
      val currentBusConfig = getGenericBusConfig(dataWidth = busDataWidthLocal)
      ...
      simConfig
        ...
        .doSim { dut =>
          ...
          assert(
            !dut.clockDomain.waitSamplingWhere(timeout)(responses.nonEmpty),
            "Timeout waiting for read response (internal > bus)"
          )
          val (_, error) = responses.dequeue()
          assert(error, "Expected error for read when internalDataWidth > busDataWidth, but no error signaled.")

          // Attempt Write
          cmdQueue.enqueue { cmd =>
            cmd.address #= testAddr; cmd.isWrite #= true; cmd.writeData #= BigInt("ABCD", 16)
          } // 16-bit data
          assert(
            !dut.clockDomain.waitSamplingWhere(timeout)(responses.nonEmpty),
            "Timeout waiting for write response (internal > bus)"
          )
          val (_, error2) = responses.dequeue()
          assert(error2, "Expected error for write when internalDataWidth > busDataWidth, but no error signaled.")
