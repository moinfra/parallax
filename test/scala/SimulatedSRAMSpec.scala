...
class SimulatedSRAMSpec extends CustomSpinalSimFunSuite {
  ...
  def testSram(config: ExtSRAMConfig, testName: String)(testLogic: SimulatedSRAM => Unit): Unit = {
    test(testName) {
      SimConfig.withFstWave.compile(new SimulatedSRAM(config)).doSim { dut =>
        // 初始化为安全状态
  ...
  val defaultConfig = ExtSRAMConfig(sramSize = 1 KiB, addressWidth = 8, dataWidth = 32, readWaitCycles = 0)
  val configWithWait = ExtSRAMConfig(sramSize = 1 KiB, addressWidth = 8, dataWidth = 32, readWaitCycles = 2)
  val byteEnableConfig = ExtSRAMConfig(sramSize = 1 KiB, addressWidth = 8, dataWidth = 32, readWaitCycles = 0)
  ...
  testSram(defaultConfig, "should write and read back a value") { dut =>
    val testAddr = 0x10
  ...
  testSram(configWithWait, "should handle read wait cycles correctly") { dut =>
    val testAddr = 0x20
  ...
  testSram(byteEnableConfig, "should handle byte enables correctly during write") { dut =>
    val baseAddr = 0x30
    ...
    val expectedValue = BigInt("11eedd44", 16) // 字节0和3保持原值，字节1和2更新
  ...
  testSram(defaultConfig, "should handle detailed byte enable patterns") { dut =>
    var testAddr = 0x40
  ...
  testSram(defaultConfig, "should pass random read/write operations") { dut =>
    ...
    for (i <- 0 until 100) {
      ...
      if (rand.nextBoolean() || !memModel.contains(addr)) {
        // 写操作
        val data = rand.nextInt() & dataMask
  ...
  testSram(configWithWait, "should maintain data integrity across different wait cycles") { dut =>
    val testData = Seq(
  ...
  testSram(defaultConfig, "should handle edge cases") { dut =>
    // 测试地址边界
    val maxAddr = (1 << dut.config.addressWidth) - 1
  ...
  testSram(defaultConfig, "should handle rapid read/write cycles") { dut =>
    val baseAddr = 0x60
    val testPattern = BigInt("a5a5a5a5", 16)
    
    // 快速连续写入
    for (i <- 0 until 10) {
      writeWord(dut, baseAddr + i, testPattern + i)
    }
    
    // 快速连续读取验证
    for (i <- 0 until 10) {
      val expected = testPattern + i
      val actual = readWord(dut, baseAddr + i)
