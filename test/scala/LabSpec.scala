...
class LabTestBench(
  ...
  lazy val commitLog = dut.commitLog
  ...
  val dSram = new SimulatedSRAM(dSramConfig, initialContent = dDataWords)
...
class LabSpec extends CustomSpinalSimFunSuite {
  test("Minimal Failing Case for SUB.W rd, rj, rd") {
    ...
    compiled.doSim { dut =>
      ...
      assert(
        finalPC == successPC,
        s"Expected PC to be 0x${failPC.toString(16)} but got 0x${finalPC.toString(16)}. It seems the bug is fixed or the trigger is different."
      )
    }
  }
  ...
  test("GPR R1-R31 Correct Read/Write and Verify Test (Bug Fixed)") {
    ...
    compiled.doSim { dut =>
      ...
      println(
        s"Simulation finished at cycle $simTime. Final PC: 0x${dut.io.commitStats.maxCommitPc.toBigInt.toString(16)}"
      )
      ...
      assert(
        finalPC == successPC,
        s"Test failed! CPU ended at wrong PC. Expected 0x${successPC.toString(16)}, got 0x${finalPC.toString(16)}"
      )
  ...
  test("Sequential Memory Write Test") {
    ...
    compiled.doSim { dut =>
      ...
      for (cycle <- 1 to maxCycles) {
        ...
        if (cycle % 1000 == 0) {
          println(
            s"Cycle $cycle: Committed ${commitStats.totalCommitted.toBigInt}, maxPC=0x${commitStats.maxCommitPc.toBigInt
                .toString(16)}"
          )
        }
      }
      ...
      assert(
        dut.io.commitStats.maxCommitPc.toBigInt <= maxExpectedPc,
        s"Max PC exceeded: max=0x${dut.io.commitStats.maxCommitPc.toBigInt.toString(16)}, expected=0x${maxExpectedPc.toString(16)}"
      )
  ...
  test("Fibonacci Test on CoreNSCSCC") {
    ...
    val compiled = SimConfig
      ...
      .withFstWave
      .compile(new LabTestBench(instructions))
  ...
  test("Multiplier Test") {
    ...
    compiled.doSim { dut =>
      ...
      println(
        s"Memory check at 0x80400000: Expected=0x${expectedValue.toString(16)}, Got=0x${actualValue.toString(16)}"
      )
      assert(
        actualValue == expectedValue,
        s"Multiplier test FAILED! Expected 0x${expectedValue.toString(16)}, but got 0x${actualValue.toString(16)}"
      )
  ...
  test("BL, PCADDU12I, XORI, MUL.W Comprehensive Test") {
    ...
    assert(
      bl_test_actual_start_idx == bl_test_entry_pc_idx,
      s"BL test PC calculation mismatch! Actual start index: $bl_test_actual_start_idx, Expected: $bl_test_entry_pc_idx"
    )
  ...
  test("Comprehensive Instruction and Memory Test") {
    ...
    compiled.doSim { dut =>
      ...
      val dataProcessingCommits = dataProcessingTestCases
        .flatMap(_.instructions)
        .length + dataProcessingTestCases.length // Instructions + st.w for each
      ...
      val timeout = cd.waitSamplingWhere(timeout = 60 * 1000)(
        dut.io.commitStats.totalCommitted.toBigInt >= verificationCommitsStart
      )
  ...
  test("Sum 1 to 100 Test on CoreNSCSCC") {
    ...
    // --- 初始化 ---
    /*00*/
    instructions += addi_w(rd = 12, rj = 0, imm = 0) // $t0 (sum) = 0
    /*04*/
    instructions += addi_w(rd = 13, rj = 0, imm = 1) // $t1 (i) = 1
    /*08*/
    instructions += addi_w(rd = 14, rj = 0, imm = 101) // $t2 (limit) = 101
    /*0C*/
    instructions += lu12i_w(rd = 4, imm = 0x80400) // $a0 = 0x80400000 (结果存储地址)

    // --- 循环体 (loop 标签在 PC=0x10) ---
    /*10*/
    instructions += add_w(rd = 12, rj = 12, rk = 13) // $t0 = $t0 + $t1 (sum = sum + i)
    /*14*/
    instructions += addi_w(rd = 13, rj = 13, imm = 1) // $t1 = $t1 + 1 (i = i + 1)
    ...
    // 偏移量 = 0x10 - 0x18 = -8 字节
    /*18*/
    instructions += bne(rj = 13, rd = 14, offset = -8) // if (i != limit) goto loop

    // --- 循环结束, 存储结果 ---
    /*1C*/
    instructions += st_w(rd = 12, rj = 4, offset = 0) // mem[$a0] = $t0 (将 sum 存入内存)
    ...
    // 偏移量 = 0x20 - 0x20 = 0
    /*20*/
    instructions += bne(rj = 0, rd = 0, offset = 0) // 无效指令，应该顺序执行
    /*24*/
    instructions += beq(rj = 0, rd = 0, offset = 0) // 无限循环以停机
    ...
    compiled.doSim { dut =>
      ...
      val expectedTotalCommitted =
        numInitInstructions + (numIterations * numLoopInstructions) + numPostLoopInstructions + numHaltInstructions
