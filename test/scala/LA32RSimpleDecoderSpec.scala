...
class LA32RSimpleDecoderSpec extends CustomSpinalSimFunSuite {
  ...
  testInstruction("OR r7, r8, r9", LA32RInstrBuilder.or(7, 8, 9)) { uop =>
    assert(uop.aluCtrl.logicOp.toEnum == LogicOp.OR)
  }

  testInstruction("AND r1, r2, r3", LA32RInstrBuilder.and(1, 2, 3)) { uop =>
    assert(uop.aluCtrl.logicOp.toEnum == LogicOp.AND)
  }

  testInstruction("XOR r4, r5, r6", LA32RInstrBuilder.xor(4, 5, 6)) { uop =>
    assert(uop.aluCtrl.logicOp.toEnum == LogicOp.XOR)
  }
  ...
  // ** r0 as Source **
  testInstruction("ADD.W r3, r0, r2", LA32RInstrBuilder.add_w(3, 0, 2)) { uop =>
    assert(uop.isValid.toBoolean)
    assert(uop.archSrc1.idx.toInt == 0)
    assert(uop.writeArchDestEn.toBoolean)
  }
  ...
  testInstruction("ADDI.W r1, r2, max_pos_imm12", LA32RInstrBuilder.addi_w(1, 2, 2047)) { uop =>
    assertImm(uop, 2047)
  }

  testInstruction("ADDI.W r1, r2, max_neg_imm12", LA32RInstrBuilder.addi_w(1, 2, -2048)) { uop =>
    assertImm(uop, -2048)
  }

  testInstruction("BEQ r1, r2, max_pos_offset16", LA32RInstrBuilder.beq(1, 2, 131070)) { uop =>
    // Offset is imm[15:0] << 2, so max raw immediate is 32767
    assertImm(uop, 131068) // 32767 << 2
  }

  testInstruction("BEQ r1, r2, max_neg_offset16", LA32RInstrBuilder.beq(1, 2, -131072)) { uop =>
    // Offset is imm[15:0] << 2, so min raw immediate is -32768
    assertImm(uop, -131072) // -32768 << 2
  }

  // 测试所有寄存器作为 ADD.W 的目标
  for (regIdx <- 0 until 32) {
    testInstruction(s"LOOP ADD.W r$regIdx, r1, r2", LA32RInstrBuilder.add_w(regIdx, 1, 2)) { uop =>
      assert(uop.isValid.toBoolean)
      assert(uop.archDest.idx.toInt == regIdx)
      assert(uop.archSrc1.idx.toInt == 1)
      assert(uop.archSrc2.idx.toInt == 2)
      // 只有当目标不是r0时才写回
      assert(uop.writeArchDestEn.toBoolean == (regIdx != 0))
    }
  }

  // 测试所有寄存器作为 ST.W 的源 (base and data)
  for (regIdx <- 1 until 32) { // Avoid r0 as base for variety
    testInstruction(s"ST.W r$regIdx, 0(r${32 - regIdx})", LA32RInstrBuilder.st_w(regIdx, 32 - regIdx, 0)) { uop =>
      assert(uop.isValid.toBoolean)
      assert(uop.uopCode.toEnum == BaseUopCode.STORE)
      assert(uop.archSrc1.idx.toInt == (32 - regIdx)) // Base
      assert(uop.archSrc2.idx.toInt == regIdx) // Data
    }
  }
